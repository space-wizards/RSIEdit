...
namespace Editor
{
    ...
    public class CircularBuffer<T> : IEnumerable<T>
    {
        ...
        public CircularBuffer(int capacity, T[] items)
        {
            ...
            {
                throw new ArgumentException($"Too many items to fit circular buffer. Maximum: {capacity}",
                    nameof(items));
            }
        ...
        public bool TryFront(out T element)
        {
            if (IsEmpty)
            {
                element = default!;
            ...
            element = _buffer[_start];
            return true;
        }

        public bool TryTakeFront(out T element)
        {
            if (TryFront(out element))
            {
                PopFront();
                return true;
            }
        ...
        public bool TryBack(out T element)
        {
            if (IsEmpty)
            {
                element = default!;
            ...
            element = _buffer[(_end != 0 ? _end : Capacity) - 1];
            return true;
        }

        public bool TryTakeBack(out T element)
        {
            if (TryBack(out element))
            {
                PopBack();
                return true;
            }
